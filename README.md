---
description: >-
  Kompletna dokumentacja z praktycznymi poradami i wskazówkami, które ułatwią
  rozpoczęcie pracy z ChatBotXL.
---

# 🏠 Witaj na stronie pomocy ChatBotXL

## :white\_check\_mark: Get Started with ChatBotXL

ChatBotXL to oprogramowanie typu SaaS do tworzenia chatbotów, które automatyzuje komunikację biznesową, oferując wiele korzyści.

Przygotowaliśmy kompletną dokumentację, aby ułatwić Ci szybkie rozpoczęcie pracy. Platforma ChatBotXL jest podzielona na następujące sekcje.

1. Rozpocznij z ChatBotXL - Pierwsze kroki
2. Zarządzanie botami – Tworzenie, edycja, zarządzanie, konfiguracja i integracja botów.
3. Skrzynka odbiorcza – Zarządzanie wszystkimi interakcjami klientów z botem.
4. Zespół – Zarządzanie Twoim zespołem.
5. Subskrypcje – Zarządzanie planem, rozliczeniami i fakturami.
6. Moje konto – Profil i inne moduły pomocnicze.
7. Powiadomienia – Zarządzanie powiadomieniami i ich ustawieniami.

Szczegółowe informacje znajdziesz, przechodząc do poniższej sekcji.

{% content-ref url="chatbotxl-resource-center/how-chatbotxl-works/" %}
[how-chatbotxl-works](chatbotxl-resource-center/how-chatbotxl-works/)
{% endcontent-ref %}

{% content-ref url="chatbotxl-resource-center/frequently-asked-questions-faqs/" %}
[frequently-asked-questions-faqs](chatbotxl-resource-center/frequently-asked-questions-faqs/)
{% endcontent-ref %}

{% content-ref url="chatbotxl-resource-center/common-errors-and-troubleshooting.md" %}
[common-errors-and-troubleshooting.md](chatbotxl-resource-center/common-errors-and-troubleshooting.md)
{% endcontent-ref %}

{% content-ref url="broken-reference" %}
[Broken link](broken-reference)
{% endcontent-ref %}

{% content-ref url="broken-reference" %}
[Broken link](broken-reference)
{% endcontent-ref %}
